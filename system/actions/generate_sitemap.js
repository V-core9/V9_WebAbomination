...
single_url_string = async (data) => {
    const { url, lastmod, changefreq, priority } = data;
    return `
            <url>
                <loc>https://v-core9.com${url}</loc>
    ...
    if (cache_item !== false ? await v_cache.cache_time_check(cache_item) : true) {
        ...
        if (in_type === 'index') {
            ...
            for (let i = 0; i < pages.length; i++) {
                ...
                resp += await single_url_string({url: page.path, lastmod: '2021-12-12', changefreq: 'monthly', priority: '1'});

                for (let j = 0; j < page.alt_paths.length; j++) {
                    resp += await single_url_string({url: page.alt_paths[j], lastmod: '2021-12-12', changefreq: 'monthly', priority: '0.8'});
                }
            }
            resp += `</urlset>`;
